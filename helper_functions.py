...
def plot_outlier_results(
    ...
    and binary classification. The colorbar is set symmetrically without stretching data.
    ...
    score_max = np.max(np.abs(data[score_col]))
    vmin, vmax = -score_max, score_max  # Ensuring symmetry
    ...
    plt.show()
    print(
        f"Classified {len(data[data[binary_col] == -1])} outliers out of {len(data)} samples"
    )
